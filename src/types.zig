...
pub const Value = union(Type) {
    ...
    pub fn decode(comptime T: Type, buf: []const u8, value: *Value) !usize {
        switch (T) {
            .s, .S => |v| {
                const len = std.mem.indexOfScalar(u8, buf, 0).?;
                value.* = @unionInit(Value, @tagName(v), buf[0..len]);
            ...
            .T, .F, .N, .I => return 0,
            .i, .f, .c, .r, .m => |v| {
                value.* = @unionInit(Value, @tagName(v), @bitCast(std.mem.readInt(u32, buf[0..4], .big)));
                return 4;
            },
            .h, .t, .d => |v| {
                value.* = @unionInit(Value, @tagName(v), @bitCast(std.mem.readInt(u64, buf[0..8], .big)));
                return 8;
            },
        }
    }
